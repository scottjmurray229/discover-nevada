 ...
 *   2. Create project "Discover Nevada"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Nevada Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'nevada'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Nevada ${year}`,
      description: `${title}\n\nFull guide: https://discovernevada.info/${slug}/`,
      tags: [`Nevada ${slug}`, 'Nevada travel', `Nevada ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Nevada ✈️ #shorts`,
      description: `${title} — Full guide: https://discovernevada.info/destinations/${slug}/`,
      tags: [`${title} Nevada`, 'Nevada travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Nevada — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Nevada for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discovernevada.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discovernevada.info/companion/

#Nevada #${title.replace(/\s+/g, '')} #NevadaTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  const tags = [
    `${title} Nevada`,
    `${title} travel guide`,
    `Nevada ${new Date().getFullYear()}`,
    `${title} things to do`,
    `where to stay ${title}`,
    'Nevada travel',
    'Nevada food',
    'Nevada budget travel',
    'Nevada tips',
  ];
  if (dest?.region) tags.push(`${dest.region} Nevada`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER NEVADA                 ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
