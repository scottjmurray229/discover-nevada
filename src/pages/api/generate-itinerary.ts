...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'casino': ['las-vegas', 'reno', 'henderson'],
    'desert': ['valley-of-fire', 'red-rock-canyon', 'las-vegas'],
    'hiking': ['red-rock-canyon', 'great-basin', 'lake-tahoe'],
    'skiing': ['lake-tahoe', 'reno'],
    'history': ['virginia-city', 'carson-city', 'boulder-city'],
    'photography': ['valley-of-fire', 'red-rock-canyon', 'lake-tahoe'],
    'nature': ['great-basin', 'lake-tahoe', 'valley-of-fire'],
    'nightlife': ['las-vegas', 'reno', 'lake-tahoe'],
    'luxury': ['las-vegas', 'lake-tahoe', 'henderson'],
    'road trip': ['las-vegas', 'valley-of-fire', 'great-basin'],
    'national park': ['great-basin', 'valley-of-fire', 'red-rock-canyon'],
    'old west': ['virginia-city', 'carson-city', 'boulder-city'],
    'gambling': ['las-vegas', 'reno', 'henderson'],
    'lake': ['lake-tahoe', 'boulder-city'],
    'show': ['las-vegas', 'reno'],
  };
  ...
  return ['las-vegas', 'red-rock-canyon', 'valley-of-fire'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Nevada AI Trip Planner. You generate detailed day-by-day itineraries for Nevada travel.
...
- Nevada context: desert heat, high desert terrain, mountain elevations, casino culture, scenic drives, vast basin-and-range geography, ski resorts near Lake Tahoe, neon-lit Strip in Las Vegas, historic mining towns like Virginia City
- Tag hotel/tour/transport items with affiliateType and affiliateSlotId for future monetization
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-las-vegas"
...
- 15+ days: 2-3 items per day, brief descriptions (under 15 words each). Group similar days (e.g. "Days 5-6: Exploring Lake Tahoe"). Only include key activities, one meal, and accommodation.
...
{
  "title": "string — trip title like '7-Day Nevada Desert & Casino Adventure'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Las Vegas'",
      "destination": "string — slug like 'las-vegas'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Bellagio Fountains', 'Fire Wave Trail', 'Hoover Dam'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Nevada trip:\n${parts.join('\n')}`;
