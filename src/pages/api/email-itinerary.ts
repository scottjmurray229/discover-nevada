...
const DEST_NAMES: Record<string, string> = {
  'las-vegas': 'Las Vegas', 'reno': 'Reno', 'lake-tahoe': 'Lake Tahoe',
  'valley-of-fire': 'Valley of Fire', 'red-rock-canyon': 'Red Rock Canyon',
  'great-basin': 'Great Basin', 'virginia-city': 'Virginia City',
  'henderson': 'Henderson', 'boulder-city': 'Boulder City', 'carson-city': 'Carson City',
};
...
function buildAffiliateUrl(type: string | null | undefined, destination: string, description: string, tp?: TripParams): string {
  ...
  if (type === 'hotel') {
    ...
    const query = hotelName ? hotelName + ' ' + destName : destName + ' Nevada';
    let url = `https://www.booking.com/searchresults.html?ss=${enc(query)}&aid=2778866&label=discovernevada`;
  ...
  if (type === 'tour') {
    ...
    return `https://www.klook.com/en-US/search/result/?query=${enc(query)}&aid=112015&sub_id=discovernevada`;
  }
  if (type === 'transport') {
    return `https://www.klook.com/en-US/search/result/?query=${enc(destName + ' transport transfer')}&aid=112015&sub_id=discovernevada`;
  }
  return `https://www.klook.com/en-US/search/result/?query=${enc(destName)}&aid=112015&sub_id=discovernevada`;
}
...
function buildHotelsComUrl(destination: string, description: string): string {
  ...
  const landingPage = encodeURIComponent(`https://www.hotels.com/Hotel-Search?destination=${encodeURIComponent(query + ' Nevada')}`);
...
<body style="margin:0;padding:0;background:#F5F0E8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:linear-gradient(135deg,#0D7377,#14B8A6);border-radius:16px 16px 0 0;padding:28px 24px;text-align:center;">
      <h1 style="color:white;font-size:22px;margin:0 0 6px;">Your Nevada Itinerary</h1>
    ...
    <div style="background:#F5F0E8;border-radius:0 0 16px 16px;padding:24px 20px;">
      ...
      <div style="text-align:center;padding:24px 0 16px;">
        <a href="https://discovernevada.info/plan/" style="display:inline-block;background:#E8654A;color:white;padding:14px 32px;border-radius:100px;text-decoration:none;font-weight:700;font-size:15px;">Generate Another Itinerary</a>
      </div>

      <p style="font-size:12px;color:#94A3B8;text-align:center;line-height:1.5;margin:0;">
        Generated by <a href="https://discovernevada.info/" style="color:#0D7377;text-decoration:none;">Discover Nevada</a> AI Trip Planner<br>
        Real prices &middot; Real experience &middot; Your Nevada adventure starts here
      </p>
    </div>

    <!-- Footer -->
    <p style="color:#94A3B8;font-size:11px;text-align:center;margin:16px 0 0;line-height:1.5;">
      <a href="https://discovernevada.info/legal/privacy/" style="color:#94A3B8;">Privacy Policy</a> &middot;
      <a href="https://discovernevada.info/legal/affiliate-disclosure/" style="color:#94A3B8;">Affiliate Disclosure</a>
    </p>
  </div>
</body>
</html>`;
...
export async function POST(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Discover Nevada <hello@discovernevada.info>',
        to: [email],
        subject: `Your Nevada Itinerary: ${it.title}`,
        html,
      }),
    });
